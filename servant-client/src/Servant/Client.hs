...
instance HasClient (Raw a m) where
  type Client (Raw a m) = H.Method -> EitherT ServantError IO (Int, ByteString, MediaType, [HTTP.Header], Response ByteString)

  clientWithRoute :: Proxy (Raw a m) -> Req -> BaseUrl -> Client (Raw a m)
